;  Use G53 for all moves. This will make it safe from G90 vs G91 mode. This only works in the G17 (XY). It will give a spindle alarm in other planes.;
;
...
(print,5400 is #<5400>, _current_tool is #<_current_tool>, _selected_tool is #<_selected_tool>)
...
o100 elseif [#<new_tool> EQ 0]; M6T0 is used to reset this ATC and allow us to start a new job
 ;move to safe z
...
o100 elseif [#<_prev_tool> EQ 0]; M6T<anything> from T0 is used for a manual change before zero'ing
  ;move to safe z
    G53G0Z[#<safe_z_mpos_mm>]
  M5; turn off spindle
  ;move to change location
...
o100 else; M6T<anything> from not T0
 ...
 ;move to safe z
  ...
  G53G0X[#<change_mpos_mm_x>]Y[#<change_mpos_mm_y>]Z[#<change_mpos_mm_z>]
  G4P0 0.1
  (MSG: Install tool #<new_tool> then resume to continue)
  M0
  ;probe the new tool
  ;move over toolsetter
      G53G0Z[#<safe_z_mpos_mm>]; move to safe z
